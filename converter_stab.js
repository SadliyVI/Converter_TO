        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            if (ctx.pendingTotal) {
                ...
                if (/^\d+(?:[.,]\d+)?(?:\s+\d+(?:[.,]\d+)?)+$/.test(t)) {
                    ...
                    pushRecord(records, buildRecord({ ctx, rowNo, kind: `${ctx.pendingTotal.kind}_VALUES`, page: p, vydel: ctx.vydel }, {
                        area: parsed.area,
            ...
            if (ctx.lastMainIndex != null && ctx.vydel != null) {
                ...
                if (prev && prev.kind === "MAIN" && prev.vydel === ctx.vydel) {
                    ...
                    if (el && isNotNote && isNotNewVydel && !hasRightMetrics) {
                        ...
                        // 1) ВСЕГДА пишем SPECIES-строку (даже без характеристик), но без дублей
                        if (!ctx.emittedLonelySpecies.has(key)) {
                            pushRecord(
                                records,
                                buildRecord({ ctx, rowNo, kind: "SPECIES", page: p, vydel: ctx.vydel }, {
                                    element: el,
                                    note: "element-only",      // можно null, если не нужно
                                    raw: `lonely:${txt}`,      // или cellsRaw(cells), если предпочитаете
                                })
                            );
                            ctx.emittedLonelySpecies.add(key);
                        }

                        // 2) В формулу добавляем ТОЛЬКО если элемента там ещё нет
                        if (formulaHasElement(prevDesc, el)) {
                            prev.raw = clampCellText(normalizeSpaces([prev.raw, `| skip-el:${el}`].join(" ")));
                            continue;
                        }
                        ...
                        prev.description = appendExtraElementToMainFormula(prevDesc, el, ctx);
                        prev.raw = clampCellText(normalizeSpaces([prev.raw, `| +el:${el}`].join(" ")));
                        continue; // НЕ записываем отдельную "TEXT"/"NOTE" строку
                    }
                }
            }
            ...
            if (kind === "MAIN" || kind === "OBJECT") {
                ...
                if (kind === "OBJECT") {
                    ...
                    if (dictDesc) {
                        cells[3] = dictDesc;
                        // чтобы не терять подробности строки — сохраняем исходный текст в note
                        const tNorm = normalizeSpaces(txt);
                        if (tNorm && _internal.normKey(tNorm) !== _internal.normKey(dictDesc)) objectNote = tNorm;
                    } else {
                        cells[3] = "";
