...
function overflowLooksLikeComposition(s) {
    ...
    if (!t) return false;
    // типичные составы: 4ОС3Б2Е1С+ИВ, 10Е+Б, 5Б5ОС и т.п.
    return /\b\d{1,2}[А-ЯЁA-Z]{1,3}(?:\d{1,2}[А-ЯЁA-Z]{1,3})*(?:\+[А-ЯЁA-Z]{1,3})*\b/.test(t);
}
function overflowLooksLikeNoteText(s) {
    const t = normalizeSpaces(s).toLowerCase();
    if (!t) return false;
    return /(подрост|подлесок|болот|дорог|земли|линейного\s+протяжения|просек|реки|ручь|км|м,|тыс\.шт\/га)/i.test(t);
}

/**
 * Разруливание overflow: состав -> description (col3), текст -> col24
 */
function applyOverflowHeuristics(cells) {
    ...
    if (overflowLooksLikeComposition(ov) && !overflowLooksLikeNoteText(ov)) {
        // состав древостоя — это часть description
        cells[3] = normalizeSpaces([cells[3], ov].filter(Boolean).join(" "));
        cells[0] = "";
        return cells;
    }

    // иначе — скорее хвост текстовой колонки
...
function isNoiseText(txt) {
    if (!txt) return true;
    if (/^\d{1,4}$/.test(txt.trim())) return true;
    if (txt.includes("----------------------------------------------------------------")) return true;
    if (/^[-]{5,}$/.test(txt)) return true;
    if (txt.includes("Категория защитности") && txt.includes("Квартал")) return true;
    if (txt.trim().startsWith(":")) return true;
    return false;
}
...
/**
 * Смягчённый фильтр: если есть "подрост/подлесок/дороги/земли..." — это текст, не режем.
 */
function looksLikeTableDataLine(cells, txt) {
    const joined = (cells.slice(1).filter(Boolean).join(" ") || txt || "").trim();
    if (!joined) return false;

    if (overflowLooksLikeNoteText(joined)) return false;

    const digits = (joined.match(/\d+(?:[.,]\d+)?/g) || []).length;
    const hasElementish = /\b[А-ЯЁA-Z]{1,3}\b/.test(joined);
    const hasComposition = /\b\d[А-ЯЁA-ZЁ]{1,3}(?:\+[А-ЯЁA-ZЁ]+)?\b/.test(joined);

    return digits >= 4 && (hasElementish || hasComposition);
}
...
function normalizeMainByRanges(cells) {
...

    {
        ...
        if (!isIntInRange(d9, 1, 150)) {
            const candidates = [9, 10, 11].map((i) => ({ i, v: getInt(i) }))
...
const EMPTY_FIELDS = {
    ...
    ageKlass: null, ageGroup: null, bonitet: null, forestType: null, polnota: null,
...
function cellsRaw(cells) {
    // показываем overflow тоже, если вдруг остался
    return clampCellText(
        cells.map((x, i) => (i === 0 ? (x ? `0:${x}` : "") : x ? `${i}:${x}` : "")).filter(Boolean).join(" | ")
    );
}
...
function classifyRow(cells, ctx) {
    ...
    if (leftText.match(/^(подрост|подлесок|Болота:|Земли линейного протяжения:|Расчистка просек)/i)) {
        ctx.inSingleTrees = false; return "NOTE";
    }
...
async function parsePdf(pdfPath, onProgress) {
    ...
    for (let p = 1; p <= totalPages; p++) {
        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            cells = applyOverflowHeuristics(cells); // NEW
            ...
            // pendingTotal (numbers line)
            if (ctx.pendingTotal && kind === "TEXT") {
                const t = txt.trim();
                if (/^\d+(?:[.,]\d+)?(?:\s+\d+(?:[.,]\d+)?)+$/.test(t)) {
                    const parsed = parseTotalsNumbersFromTwoLines(ctx.pendingTotal.titleTxt, t);
                    pushRecord(records, buildRecord({ ctx, rowNo, kind: ctx.pendingTotal.kind, page: p }, {
                        area: parsed.area,
            ...
            // totals headers
            if (kind === "TOTAL_CATEGORY" || kind === "TOTAL_QUARTER") {
                ctx.pendingTotal = { kind, titleTxt: txt, page: p, rowNo };
                continue;
            }
            if (kind === "TOTAL_SPECIES_HEADER") {
                pushRecord(records, buildRecord({ ctx, rowNo, kind, page: p }, { note: "По составляющим породам", raw: txt }));
                continue;
            }
            ...
            if (kind === "OBJECT") {
                ...
                if (desc && !looksLikeRealObjectName(desc) && !hasMainMetrics) {
                    ...
                    ctx.pendingMain = { vydel: vyd, area: ar, description: desc, page: p, rowNo };
            ...
            if (ctx.pendingMain && (kind === "TEXT" || kind === "OBJECT")) {
                ...
                if (hasMainMetrics && looksLikePolnota(cells)) {
                    ...
                    cells[3] = looksLikeStandDescription(d0) ? d1 : normalizeSpaces(`${d0} ${d1}`);
            ...
            if (kind === "NOTE" || kind === "TEXT") {
                ...
                // Ключевая правка: note берём из txt (порядок слева-направо), а не из cells
                const note = txt;
